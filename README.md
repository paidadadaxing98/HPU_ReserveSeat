...
调度服务会在每天前一天 `19:30` 按账号顺序运行次日任务。账号尚未初始化时会安全停止并提示先初始化，不会进入选座或提交流程。当前企业微信机器人是单向通知，尚未接入机器人入站命令或微信小程序双向控制。
...
主服务的定时调度已支持多账号 dry-run、初始化前置检查、配额控制、真实预约入口和企业微信结果通知。真实无人值守运行仍需在当前校园网络中完成每个账号的初始化和页面校准；任何初始化失败、认证缺失、页面结构变化或核验不明确都会停止，不会自动重复提交。
