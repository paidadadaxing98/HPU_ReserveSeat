...
- 默认预览模式，不提交真实预约。
- 真实预约需要命令参数 `--submit`，并在终端再次输入大写 `SUBMIT`。
...
加上 `--submit` 后仍需在终端输入 `SUBMIT` 才会提交。测试时应先使用预览模式，并确认页面实际显示的日期、开始时间、结束时间和座位号。
...
当前最重要的未闭环问题仍是网站时间控件和结束时间接口：一次打开座位窗口可能先发出 `/endTimesForSeat/.../now`，点击开始时间后再发出带具体开始时间 `id` 的请求。预览脚本现在只接受路径和 query 的 `start` 都匹配所选开始时间 `id` 的原生响应，并保留原生响应 URL 和 `endTimes` 选项；如果页面没有发出匹配响应，会停止而不是猜测。该逻辑已通过本地回归测试，但还需要在可访问校园网站的环境中用真实预览验证，未完成联网验证前不应无人值守开启真实预约。
...
- 当前手机控制页和 `/api/v1` 接口是本地服务，不是公网服务，也没有企业微信小程序双向消息通道。
...
3. 增加预约结果、异常、需要人工处理时的企业微信或微信机器人通知。
...
真实预约前建议顺序：先跑自动化测试，再运行预览模式，确认页面上显示的日期/时间/座位，最后才使用 `--submit` 并输入 `SUBMIT`。
