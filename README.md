...
```powershell
Get-ScheduledTask -TaskName "SeatAssistant-Morning","SeatAssistant-Afternoon","SeatAssistant-Evening"
```
...
| 任务                        | 默认启动时间    | 预约对象 |
| ------------------------- | ---------:| ---- |
| `SeatAssistant-Morning`   | 前一天 22:00 | 次日上午 |
...
程序只加载 `enabled: true` 的账号；`enabled: false` 的账号不会校验账号密码、不会创建会话、不会登录或预约，因此可以保留待以后启用的账号信息。至少需要一个已启用账号。启用账号后，程序会自动为其创建独立的浏览器会话和数据库目录。
