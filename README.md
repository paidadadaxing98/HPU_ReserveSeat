...
使用 `--submit` 完成真实预约后，脚本会捕捉提交成功/失败页面提示，并尝试关闭成功界面，再进入“我的预约”核验；核验会合并当天历史和当前预约记录，只有完全匹配的 `RESERVE` 记录才判定成功，并发送企业微信通知。当前真实网站的成功弹出层仍可能未稳定关闭，导致后续导航卡住，因此登录和成功弹出层都仍属于 v0.xx 的未闭环问题。若当天已有其他有效预约，会明确报告并停止；如果提交超时或预约接口迟迟没有匹配记录，也会发送结果不明确提醒。未加 `--submit` 的预览不会发送通知。
...
当前最重要的未闭环问题仍是登录自动化和预约成功弹出层关闭：登录受 CAS 动态会话、跳转和验证码影响；预约成功文字可以捕捉，但自定义成功弹出层仍可能残留并遮挡“我的预约”导航。未重新验证前不应无人值守开启真实预约。
