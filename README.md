...
机器人收到取消、推迟等修改类命令后会立即回复“已写入本地数据库，等待动态监控受理”；实际取消或调整操作在当天对应动态监控的下一轮执行。若 2 分钟后**推迟**命令仍无人受理（例如该时段动态监控已结束），机器人进程会自己开浏览器补执行并回传结果；若该账号当天已存在有效预约，**取消**命令也会由机器人兜底远程取消。**取消命令优先由动态监控受理**（取消需同步该时段的动态会话状态）。

**按日期取消**：`取消MM-DD【上午/下午/晚上】`（如 `取消09-08上午`，也可写完整日期 `取消2026-09-08上午`；省略时段则取消当天全部时段）。日期命令写入时即绑定目标日期：如果预约还没提交，对应日期的定时预约任务会在预约前跳过该时段并回执“已按要求取消”；如果预约已存在，则由动态监控或机器人兜底远程取消。日期范围限今天起 7 天内。
...
“启用账号 / 关闭账号”改变账号的 `enabled` 状态并写入 `accounts.json`：不带账号名时对自己绑定的账号生效，也可写 `关闭账号 <账号或别名>` 指定目标。关闭立即生效：后续的定时预约任务不再包含该账号，正在运行的动态监控会在下个轮询周期自动停止；同时机器人会自动取消该账号当日剩余的预约并推送结果，避免无人到馆产生违约记录。启用后对下次启动的预约/监控任务生效，已在运行的监控不会自动恢复。被平台限制预约的账号建议先关闭，解禁后再启用。
