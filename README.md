...
2. 按用户可调整的到馆区间和离馆区间规划上午、下午、晚上的学习时段。
...
- 选择阅览室前会读取“当前预约”；只有接口明确标记为有效的预约状态（例如 `RESERVE`、`RESERVED`、`CONFIRMED`）才参与判断。同一天、阅览室不冲突且已预约时间覆盖请求时段至少 75% 时，程序会直接复用已有预约并停止，不会取消或重新预约；取消、过期、完成、释放或缺少状态的历史条目会被忽略。
- 读取座位的可选开始时间和结束时间。
...
`SEAT_WECOM_WEBHOOK` 可选。配置后，每个时段的预约成功、明确失败或结果不明确都会发送一条文本消息到企业微信。Webhook 属于敏感凭据，只保存在本机 `.env`，不要粘贴到聊天、日志或 Git 提交中；如果地址曾经暴露，应在企业微信后台删除旧机器人并重新生成。
...
使用 `--submit` 完成真实预约后，脚本会尝试自动关闭提交成功界面，再进入“我的预约”核验，最后发送企业微信通知；如果提交超时或核验结果不明确，也会发送一条结果不明确的提醒。首次真实测试中成功弹窗仍曾需要人工关闭，代码已补强但尚未在登录会话过期后重新联网验证。未加 `--submit` 的预览不会发送通知。

为减少不必要的取消和重新预约，脚本在进入阅览室前及点击“立即预约”前都会检查当前预约。请求 `15:00-17:00` 时，只有带有效状态的 `15:30-17:30` 才会按 75% 重合度被视为可继续使用；取消、过期或无状态的同一条目不会阻止新的预约流程。有效预约 `16:00-18:00` 与请求的重合度仅为 50%，也不会阻止新的预约流程。
...
当前最重要的未闭环问题仍是网站时间控件和结束时间接口：一次打开座位窗口可能先发出 `/endTimesForSeat/.../now`，点击开始时间后再发出带具体开始时间 `id` 的请求。预览脚本现在只接受路径和 query 的 `start` 都匹配所选开始时间 `id` 的原生响应，并保留原生响应 URL 和 `endTimes` 选项；如果页面没有发出匹配响应，会停止而不是猜测。该逻辑已通过本地回归测试；`20:00-21:00` 的真实预约链路已成功完成过一次，但弹窗自动关闭补强和新的 `21:00-22:00` 测试尚未联网复核，原因是浏览器登录会话已过期。登录验证码/会话续期暂留待后续处理，未重新验证前不应无人值守开启真实预约。
