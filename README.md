...
- **验证结果**：本地自动化测试 `266 passed`，`compileall` 和 `git diff --check` 通过；Windows 计划任务、常驻进程和企业微信入站机器人本轮未修改。真实网站仍需在校园网络和实际预约窗口复核。
...
- 服务在每天 `19:30` 按账号顺序运行次日任务。每次运行每个账号只寻找一个当前可执行的时段：前一预约未结束则返回 `waiting`，已结束后下一次运行继续；成功后返回 `progressed`，不把尚未执行的时段误标记为完成。只有所有启用时段都预约成功才返回 `completed`；明确失败或错过窗口会记录为终态并继续后续时段，结果不明确会返回 `uncertain` 并冻结后续提交。`SEAT_MAX_RESERVATIONS_PER_RUN` 应保持默认值 `1`。
...
项目已有覆盖配置、命令、日期选择、调度、座位解析、预览、提交保护、本地服务状态和 HTTP 接口的自动化测试。本轮本地自动化测试为 `266 passed`，业务闭环使用 dry-run 适配器验证；真实网站仍需在可访问校园网络且完成统一认证验证码的环境中运行预览，不能仅依据本地测试判断真实页面成功。
...
调度服务会在每天前一天 `19:30` 按账号顺序运行次日任务。使用 `accounts.json` 的账号尚未初始化时会安全停止并提示先初始化，不会进入选座或提交流程；不创建 `accounts.json` 时，旧的 `.env` 单账号模式默认使用“南校区第二图书馆”，可以直接运行。当前企业微信机器人是单向通知，尚未接入机器人入站命令或微信小程序双向控制。Windows 计划任务和常驻进程本轮没有修改。
