...
当前最重要的未闭环问题仍是网站时间控件和结束时间接口：旧校准数据表明原生结束时间路径使用服务端返回的开始时间 `id`（例如 `now`），不能简单把显示时间手工转换成分钟。预览脚本现在点击开始时间后直接捕获页面发出的原生 `endTimesForSeat` 响应，并保留原生响应 URL 和 `endTimes` 选项；如果页面没有发出该响应，会停止而不是猜测。该逻辑已通过本地回归测试，但还需要在可访问校园网站的环境中用真实预览验证，未完成联网验证前不应无人值守开启真实预约。
