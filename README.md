...
- 选择阅览室前会读取“当前预约”；同一天、阅览室不冲突且已预约时间覆盖请求时段至少 75% 时，程序会直接复用已有预约并停止，不会取消或重新预约。
- 读取座位的可选开始时间和结束时间。
...
为减少不必要的取消和重新预约，脚本在进入阅览室前及点击“立即预约”前都会检查当前预约。请求 `15:00-17:00` 时，已有 `15:30-17:30` 的重合度为 75%，会被视为可继续使用；已有 `16:00-18:00` 的重合度仅为 50%，不会阻止新的预约流程。
